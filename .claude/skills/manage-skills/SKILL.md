...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-react-patterns` | React 컴포넌트 구조, hooks 사용 패턴, 명명 규칙 검증 | `src/**/*.jsx`, `src/**/*.js` |
| `verify-supabase` | Supabase 사용 패턴, 에러 처리, 쿼리 최적화 검증 | `src/lib/supabase.js`, `src/**/*.jsx` |
| `verify-auth` | 인증 및 보안 패턴, ProtectedRoute 사용, 권한 체크 검증 | `src/App.jsx`, `src/pages/**/*.jsx`, `src/components/ProtectedRoute.jsx` |
| `verify-imports` | import 순서, 경로 일관성, 사용하지 않는 import 검증 | `src/**/*.jsx`, `src/**/*.js` |
